      ...
      *                                                                *
      *                                                                *
      *              ADD Policy  ─  VSAM KSDS 存取層                   *
      *                                                                *
      * 原始程式: LGAPVS01.cbl (CICS/VSAM)                             *
      * 轉置目標: ACUCOBOL-GT v11.0                                    *
      *                                                                *
      * 轉置摘要:                                                       *
      *   EXEC CICS Write File('KSDSPOLY') → WRITE 陳述式             *
      ...
      *---------------------------------------------------------------*
      * 從 COMMAREA 組裝保單記錄
      *   原始: CA-Request-ID(4:1) 取得保單類型字元
      *   WF-Policy-Key 作為 RIDFLD（複合鍵）
      *---------------------------------------------------------------*
      ...
      *---------------------------------------------------------------*
      * WRITE 對應 EXEC CICS Write File('KSDSPOLY')                   *
      *   原始 LENGTH(64)，記錄定義為 64 bytes                         *
      *---------------------------------------------------------------*
           OPEN OUTPUT KSDSPOLY-FILE.
           IF NOT KSDSPOLY-OK
               MOVE '80' TO CA-RETURN-CODE
               MOVE 1    TO WS-RESP
               PERFORM WRITE-ERROR-MESSAGE
               DISPLAY 'LGAPVS01: OPEN KSDSPOLY failed '
                       WS-KSDSPOLY-STATUS UPON SYSERR
               STOP RUN
           END-IF.

           WRITE KSDSPOLY-RECORD.
